<?php 
	...
	class Mod_Query extends CI_Model
	{
		...
		public function get_where($ctrl,$table_name,$data,$limit='',$field='',$direct='',$group='')
		{
			$query = $this->db->limit($limit)->order_by($field,$direct)->group_by($group)->get_where($table_name,$data);
			switch ($ctrl) {
				case 'row':
					return $query->row();
					break;
		...
		public function get($ctrl,$table_name,$limit='',$field='',$direct='',$group='')
		{
			$query = $this->db->limit($limit)->order_by($field,$direct)->group_by($group)->get($table_name);
			switch ($ctrl) {
				case 'row':
					return $query->row();
					break;
